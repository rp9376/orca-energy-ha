...
class OrcaClimate(OrcaEntity, ClimateEntity):
    ...
    def __init__(self, coordinator: OrcaDataUpdateCoordinator, circuit_id: int) -> None:
        ...
        super().__init__(coordinator, self._get_unique_id("hc_name"))

        # support for both languages
        hc_name_data = self.coordinator.data.get(self._get_unique_id("hc_name"))
        eng_name: str = (
            hc_name_data.value if hc_name_data else f"Heating Circuit {circuit_id}"
        )
        if coordinator.config_entry.data.get(CONF_LANGUAGE, LANG_EN) == LANG_SI:
            self._attr_name = str(CIRCUIT_NAME_MAP_SI.get(eng_name, eng_name)).title()
        else:
